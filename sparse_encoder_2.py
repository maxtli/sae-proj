...
for i,batch in enumerate(tqdm(iter(owt_loader))):
    ...
    if i >= 20:
        act_list = torch.cat(activation_storage, dim=0)
        break
    # if i % 10 == 9:
...
U, S, V = torch.pca_lowrank(act_list[:,1:].flatten(0,1),768)

# %%
explained_var = (torch.pow(S, 2) / torch.sum(torch.pow(S, 2))).cumsum(dim=0)
sns.lineplot(x=range(explained_var.shape[0]), y=explained_var.cpu().numpy())
plt.show()

sns.lineplot(x=range(50), y=explained_var[:50].cpu().numpy())
plt.show()
...
for i,batch in enumerate(tqdm(iter(owt_loader))):
    ...
    if i % -1000 == -1:
        torch.save(sae.state_dict(),f"SAE_training/SAE_untied_2/epoch_{j}.pt")
        if len(agg_losses) > 0:
            q=.95
            ...
            plt.show()
            with open(f"SAE_training/SAE_untied_2/loss_graph.pkl", "wb") as f:
...
# %%
# with open("SAE_training/SAE_untied_2/loss_graph.pkl", "rb") as f:
...
rand_directions = t.normal(0,1,(feature_dim,activation_dim)).to(device)
rand_directions /= rand_directions.norm(dim=-1,keepdim=True)

# %%

with open(f"init_sae/v3/feature_{100}.pkl", "rb") as f:
    rand_directions = pickle.load(f)

# %%

rand_directions = V[:,:100].permute(1,0)

# %%
rand_directions = model.W_pos[:25].detach()
rand_directions /= rand_directions.norm(dim=-1,keepdim=True)

# %%

center = sae.floating_mean
# center = acts.mean(dim=0)
# %%
# normalized_acts = (acts / acts.norm(dim=-1, keepdim=True)).flatten(0,1)
normalized_acts = ((acts - center) / (acts - center).norm(dim=-1, keepdim=True)).flatten(0,1)
with torch.no_grad():
    # cos_sims_feat_act = einsum("feature activation, batch activation -> batch feature", sae.feature_weights, normalized_acts)
    cos_sims_feat_act = einsum("feature activation, batch activation -> batch feature", rand_directions, normalized_acts)
    # cos_sims_feat_act = einsum("feature activation, batch activation -> batch feature", rand_directions, rand_directions)

# %%
sns.histplot(cos_sims_feat_act[:1000].flatten().cpu().numpy())
plt.show()

# %%
# PCA directions are ordered
sns.histplot(cos_sims_feat_act[:10000,:10].cpu().numpy())
plt.show()

# %%
sns.histplot(torch.topk(cos_sims_feat_act[:10000], 10, dim=-1)[0].cpu().numpy())
# %%
sns.histplot((cos_sims_feat_act-.1).relu().sum(dim=-1).cpu().numpy())

# %%
sns.histplot((cos_sims_feat_act > .1).sum(dim=0).log().flatten().cpu().numpy())
# %%
sns.histplot((cos_sims_feat_act.relu()).mean(dim=0).flatten().cpu().numpy())

# %%

sns.histplot(torch.bincount(torch.argmax(cos_sims_feat_act[:1000], dim=-1)).log().cpu(), bins=100)

# %%

# model.W_pos.shape[:25]
